...
exports.getJobStatus = async (req, res) => {
  try {
    ...
    const response = {
      ...
      rawVideoUrl: session.videoAssetId
        ? session.videoAssetId.cloudinaryUrl
        : null,
      highlights: [],
      clips: [],
...
const processJobAsync = async (sessionId) => {
  ...
  try {
    ...
    console.log(
      `[JOB ${sessionId}] Step 1 DONE: Video downloaded (${fs.statSync(localVideoPath).size} bytes)`,
    );
    ...
    try {
      ...
      console.log(
        `[JOB ${sessionId}] Video duration: ${videoDuration.toFixed(1)}s`,
      );
    } catch (err) {
    ...
    console.log(
      `[JOB ${sessionId}] Step 2 DONE: Audio extracted (${(fs.statSync(audioPath).size / 1024 / 1024).toFixed(2)} MB)`,
    );

    try {
      const audioDuration = await ffmpegService.getMediaDuration(audioPath);
      console.log(
        `[JOB ${sessionId}] Audio duration: ${audioDuration.toFixed(1)}s`,
      );
    } catch (err) {
    ...
    console.log(
      `[JOB ${sessionId}] Step 3 DONE: ${transcription.segments.length} segments, ${transcription.text.length} chars`,
    );

    if (isTranscriptTooShort(videoDuration, transcription)) {
      console.log(
        `[JOB ${sessionId}] Transcript too short for video length, retrying with WAV...`,
      );
      const wavPath = path.join(tempDir, `audio_${sessionId}.wav`);
      await ffmpegService.extractAudioWav(localVideoPath, wavPath);
      transcription = await transcriptionService.transcribeAudioWithTimestamps(
        wavPath,
        {
          durationHint: videoDuration,
          forceChunking: true,
      ...
      console.log(
        `[JOB ${sessionId}] Retry DONE: ${transcription.segments.length} segments, ${transcription.text.length} chars`,
      );
    }
    ...
    console.log(
      `[JOB ${sessionId}] Step 4 DONE: Found ${highlights.length} highlights`,
    );
    ...
    for (let i = 0; i < highlights.length; i += 1) {
      console.log(
        `[JOB ${sessionId}] Step 5: Trimming clip ${i + 1}/${highlights.length}...`,
      );
      const highlight = highlights[i];
      const range =
        typeof highlight.start === "number" && typeof highlight.end === "number"
    ...
    console.log(
      `[JOB ${sessionId}] Step 6: Creating final 1-min highlight reel...`,
    );
    ...
    console.log(
      `[JOB ${sessionId}] Step 6 DONE: Reel uploaded -> ${reelUpload.secure_url}`,
    );
