...
const transcribeAudioWithTimestamps = async (filePath, options = {}) => {
    try {
        const stats = fs.statSync(filePath);
        console.log(`Transcribing audio with timestamps: ${filePath} (${(stats.size / 1024 / 1024).toFixed(2)} MB)`);

        const durationHint = typeof options.durationHint === 'number' ? options.durationHint : 0;
        ...
        if (durationHint > 0 && (duration === 0 || Math.abs(duration - durationHint) > 5)) {
        ...
        if (!forceChunking && (duration <= MAX_CHUNK_DURATION || duration === 0)) {
            ...
            console.log(`Got ${result.segments.length} segments, ${result.text.length} chars`);

            if (
                duration > MIN_LONG_AUDIO_SECONDS &&
                (result.text.length < MIN_TEXT_CHARS_FOR_LONG_AUDIO || result.segments.length < MIN_SEGMENTS_FOR_LONG_AUDIO)
            ) {
                console.log('Transcript looks too short for audio length, retrying with chunking...');
                return transcribeAudioWithTimestamps(filePath, { durationHint, forceChunking: true });
            }
            return result;
        }

        // For longer audio, split into chunks and transcribe each
        console.log(`Audio is ${duration.toFixed(1)}s — splitting into ${Math.ceil(duration / MAX_CHUNK_DURATION)} chunks...`);
        const ffmpegService = require('./ffmpegService');
        ...
        while (offset < duration) {
            ...
            console.log(`  Chunk ${chunkIndex + 1}: ${offset.toFixed(1)}s - ${(offset + chunkDuration).toFixed(1)}s`);
            await ffmpegService.splitAudio(filePath, offset, chunkDuration, chunkPath);
        ...
        console.log(`Total transcription: ${allSegments.length} segments, ${allText.length} chars`);
